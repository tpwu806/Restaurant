...
public class EmpLogin extends JPanel implements ActionListener{
	...
	public EmpLogin()
	{
		...
		String []params={"1"};
		String sql="select login.empid,empname,passwd,zhiwei from login,rszl where login.empid=rszl.empid and 1=?";
		em=new EmpModel();
		em.query(sql, params);
...
//		jtable.setModel(em);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(arg0.getSource().equals(p1_jb))
		{
			...
			String sql="select login.empid,empname,passwd,zhiwei from login,rszl where login.empid=rszl.empid and login.empid=?";
			em=new EmpModel();
			em.query(sql, params);
